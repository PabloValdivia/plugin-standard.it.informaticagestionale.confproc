 ...
 * inverted production: from a good to components 
 * @author Layda Salas (globalqss) Giorgio Cafasso
 * @version $Id: M_ProductionDrop_Run,v 1.0 2005/10/09 22:19:00 globalqss Exp $
 */
public class M_ProductionDrop_Run extends SvrProcess {
		...
		if (!isCreated) {
			...
			while (pp.next()) {
				...
				sqlins = "INSERT INTO M_ProductionLine "
						...
						+ pp.getInt("ProductionQty")*-1 + ", " //inverted production 
				...
				while (bom.next()) {
					...
					sqlins = "INSERT INTO M_ProductionLine "
							...
							+ ","
							+ - (pp.getInt("ProductionQty") * bom	//signus inverted for production drop
									.getInt("BOMQty")) + ","
							+ pp.getInt("M_Locator_ID") + ")";
					cnti = DB.executeUpdate(sqlins, get_TrxName());
					if (cnti == -1)
